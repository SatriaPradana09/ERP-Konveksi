...
@section('content')
    <div class="container-fluid mb-3">
        <div class="form-group mt-4">
            <form action="{{ url('/Akun/SQ/tanggal/') }}" method="POST" id="searchForm">
                {{ csrf_field() }}
    ...
    <div class="card">
        ...
        <script>
            function cetakTabel() {
                ...
                printWindow.document.write('<h2 class="text-center mb-4">Customer Bill</h2>');
                printWindow.document.write('<table class="table table-bordered">');
                printWindow.document.write('<thead>');
                printWindow.document.write('<tr>');
                printWindow.document.write('<th scope="col">No</th>');
                printWindow.document.write('<th scope="col">Nama Pengguna</th>');
                ...
                @if ($sqs->count())
                    @foreach ($sqs as $item)
                        ...
                        printWindow.document.write('<td>{{ $item->tanggal_transaksi }}</td>');
